...
//  pArea = reinterpret_cast<AREA_DATA *>(alloc_perm (sizeof (*pArea)));
  ...
  else pArea->area_flags = fread_flag(fp);
...
//      pHelp = reinterpret_cast<HELP_DATA *>(alloc_perm(sizeof(*pHelp)));
      pHelp->level = level;
...
//	      pexit = reinterpret_cast<EXIT_DATA *>(alloc_perm(sizeof(*pexit)));
	      ...
	      else pexit->exit_info = fread_number (fp);
	  ...
	  if (HAS_SCRIPT (pMob))
	      ...
	      for (pTrig = pMob->triggers; pTrig != NULL; pTrig = pTrig->next)
		{
		  if (pTrig->trigger_type ==
		      TRIG_BORN && pTrig->current ==
		      NULL && !IS_SET (pMob->act, ACT_HALT))
		    {
...
/*      if (!strcmp(pArea->name,"`gStillwater``")) {
       ...
       */
      if ((!pArea->empty
	   && (pArea->nplayer == 0 || pArea->age >= 15)) || pArea->age >= 62)
	{
	  ROOM_INDEX_DATA *pRoomIndex;
	  reset_area (pArea);
  ...
  for (pReset = pArea->reset_first; pReset != NULL; pReset = pReset->next)
	...
	case 'O':
	  ...
	  if ((pObjIndex->item_type == ITEM_CTRANSPORT
	       || pObjIndex->item_type ==
	       ITEM_ELEVATOR) && pObjIndex->count > 0)
	    {
	      last = FALSE;
	      break;
	    }
...
/*      if (obj->pIndexData->vnum == OBJ_VNUM_COMMON_CARD)
	...
	{
	  case 1: sprintf(buf2,"sword");obj->item_type = ITEM_WEAPON;obj->wear_flags = ITEM_WIELD;obj->value[0] = WEAPON_SWORD;break;
	  case 2: sprintf(buf2,"helmet");obj->wear_flags = ITEM_WEAR_HEAD;break;
	  case 3: sprintf(buf2,"bracer");obj->wear_flags = ITEM_WEAR_WRIST;break;
          case 4: sprintf(buf2,"ring");obj->wear_flags = ITEM_WEAR_FINGER;break;
	  case 5: sprintf(buf2,"necklace");obj->wear_flags = ITEM_WEAR_NECK;break;
	  case 6: sprintf(buf2,"set of leg guards");obj->wear_flags = ITEM_WEAR_LEGS;break;
	  case 7: sprintf(buf2,"pair of boots");obj->wear_flags = ITEM_WEAR_FEET;break;
	  case 8: sprintf(buf2,"pair of gloves");obj->wear_flags = ITEM_WEAR_HANDS;break;
	  case 9: sprintf(buf2,"cape");obj->wear_flags = ITEM_WEAR_ABOUT;break;
	  case 10: sprintf(buf2,"belt");obj->wear_flags = ITEM_WEAR_WAIST;break;
          case 11: sprintf(buf2,"gown");obj->wear_flags = ITEM_WEAR_ABOUT;break;
          case 12: sprintf(buf2,"breastplate");obj->wear_flags = ITEM_WEAR_BODY;break;
          default: sprintf(buf2,"pair of pants");obj->wear_flags = ITEM_WEAR_LEGS;break;
	}
	...
	{
  	  case 1: sprintf(buf3,"red");
          case 2: sprintf(buf3,"black");
	  case 3: sprintf(buf3,"brown");
	  case 4: sprintf(buf3,"grey");
	  case 5: sprintf(buf3,"white");
	  case 6: sprintf(buf3,"dark blue");
	  case 7: sprintf(buf3,"dark red");
	  case 8: sprintf(buf3,"dark green");
	  case 9: sprintf(buf3,"dark brown");
	  case 10: sprintf(buf3,"dark teal");
	  default: sprintf(buf3,"dark beige");
	}
	switch (number_range(1,4))
        {
	  case 1: sprintf(buf,"a half decayed %s %s", buf3, buf2);
	  case 2: sprintf(buf,"a partially digested %s %s", buf3, buf2);
	  case 3: sprintf(buf,"a mutilated %s %s", buf3, buf2);
	  default : sprintf(buf,"a worthless %s %s", buf3, buf2);
	}
...
void clone_object (OBJ_DATA * parent, OBJ_DATA * clone)
{
  ...
  else clone->plr_owner = str_dup(parent->plr_owner);
...
static inline void format_color (char *newstr, char *str, int length)
{
  char *cptr;
  int tcnt = 0;
