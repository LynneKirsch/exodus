...
void acid_effect (void *vo, int level, int dam, int target)
{
  ...
  if (target == TARGET_CHAR)
    {				/* do the effect on a victim */
      ...
      OBJ_DATA *obj, *obj_next;
      if (victim->dueler || battle_royale)
        return;
  ...
  if (target == TARGET_OBJ)
    {				/* toast an object */
      ...
      int chance;
      char *msg;
      if (IS_OBJ_STAT (obj, ITEM_BURN_PROOF) || IS_SET(obj->extra_flags[1],ITEM_ACID_PROOF)
...
{
  if (target == TARGET_ROOM && !battle_royale && superduel_ticks < 10)
    {				/* nail objects on the floor */
      ROOM_INDEX_DATA *room = (ROOM_INDEX_DATA *) vo;
      OBJ_DATA *obj, *obj_next;
      for (obj = room->contents; obj != NULL; obj = obj_next)
  ...
  if (target == TARGET_CHAR)
	...
	gain_condition (victim, COND_HUNGER, dam / 20);
      if (!battle_royale && !(victim->dueler))
  ...
  if (target == TARGET_OBJ)
    {				/* toast an object */
      ...
      int chance;
      char *msg;
      if (IS_OBJ_STAT (obj, ITEM_BURN_PROOF) || IS_SET(obj->extra_flags[1],ITEM_COLD_PROOF)
	  || IS_OBJ_STAT (obj, ITEM_NOPURGE) || number_range (0, 4) == 0)
	return;
      chance = level / 4 + dam / 10;
      if (chance > 25)
...
void poison_effect (void *vo, int level, int dam, int target)
{
  ...
  if (target == TARGET_OBJ)
    {				/* do some poisoning */
      OBJ_DATA *obj = (OBJ_DATA *) vo;
      int chance;
      if (IS_OBJ_STAT (obj, ITEM_BLESS) || number_range (0, 4) == 0)
	return;
      chance = level / 4 + dam / 10;
      if (chance > 25)
	chance = (chance - 25) / 2 + 25;
  ...
  if (target == TARGET_ROOM)
    {
      ROOM_INDEX_DATA *room = (ROOM_INDEX_DATA *) vo;
      OBJ_DATA *obj, *obj_next;
      if (!battle_royale && superduel_ticks < 10)
	{
	  for (obj = room->contents; obj != NULL; obj = obj_next)
	    {
	      obj_next = obj->next_content;
  ...
  if (target == TARGET_OBJ)
    {
      ...
      int chance;
      char *msg;
      if (IS_OBJ_STAT (obj, ITEM_BURN_PROOF) || IS_SET(obj->extra_flags[1],ITEM_LIGHTNING_PROOF)
	  || IS_OBJ_STAT (obj, ITEM_NOPURGE) || number_range (0, 4) == 0)
	return;
      chance = level / 4 + dam / 10;
      if (chance > 25)
