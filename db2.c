...
//    trigger->script = reinterpret_cast<SCRIPT_DATA *>(alloc_perm(sizeof(*scr)));
    scr = trigger->script;
    scr->command = str_dup ("");
    scr->next = NULL;
  }
...
//      word = feof (fp) ? const_cast<char *>("End") : fread_word (fp);
	...
	case 'S':
	  if (!str_cmp (word, "Sc") || !str_cmp (word, "Script"))
		...
		{
		  for (scr = trigger->script; scr->next != NULL; scr = scr->next);	/* scan to the end */
		  scr->next = alloc_perm (sizeof (*scr));
//		  scr->next = reinterpret_cast<SCRIPT_DATA *>(alloc_perm(sizeof(*scr)));
		  scr = scr->next;
		}
	      else
		{
...
//      pMobIndex = reinterpret_cast<MOB_INDEX_DATA *>(alloc_perm (sizeof (*pMobIndex)));
      pMobIndex->vnum = vnum;
...
//        pMobIndex->hitroll = pMobIndex->level;
      ...
      /*10/12/03 - Iblis - Added this to properly load new flags Cory added a couple months ago*/
	...
	else pMobIndex->number_of_attacks = -1;
      }
      else
      {
...
//       fprintf (stderr, "!! - %d - %d - %ld - %d - %d - %d \n", pMobIndex->ac[AC_EXOTIC], pMobIndex->blocks_exit, pMobIndex->vuln_flags,
//		       pMobIndex->start_pos, pMobIndex->default_pos, pMobIndex->sex);
      ...
      //Iblis - 8/29/04 - Added a slot for a rune card vnum, and setable stats (con/str/etc)
...
//      pObjIndex = reinterpret_cast<OBJ_INDEX_DATA *>(alloc_perm (sizeof (*pObjIndex)));
      pObjIndex->vnum = vnum;
...
#ifdef VERBOSE_BOOT
      sprintf (buf, "Loading object: %s vnum: %d", pObjIndex->name,
	       pObjIndex->vnum);
      log_string (buf);
...
#ifdef VERBOSE_BOOT
      sprintf (buf,
	       "load_objects: read type name: \"%s\" item_lookup: %d",
	       riname, pObjIndex->item_type);
      log_string (buf);
        ...
        for (i=0;i<x;i++)
	{
      ...
      else  pObjIndex->extra_flags[0] = fread_flag (fp);
      pObjIndex->wear_flags = fread_flag (fp);

      // Akamai 4/30/99 - support for Class/race specific equipment
      // If the version of the current area is a version supporting
      // Class/race specific eq then load those flags
...
#ifdef VERBOSE_BOOT
	  log_string ("load_objects: objects have Class/race flags");
...
#ifdef VERBOSE_BOOT
        log_string ("load_objects: objects have an obj_trigger vnum");
#endif /*  */
      }
...
#ifdef VERBOSE_BOOT
	  sprintf (buf, "load_objects: %s item %d -> a %s",
	...
	case ITEM_CLAN_DONATION:	// new_clans
	  // this is paired with the the save_object write in olc_save.c
	  //
	  // note this works because if save_object did *not* already
	  // save this donation box in the new format then this case is
	  // not taken and the default case is used - however, if save_object
	  // saved this box already once then this new object type is set
	  // and this case will be taken, reading the name in the new way
	...
	case ITEM_NEWCLANS_DBOX:	// new_clans 
      ...
      pObjIndex->value[5] = fread_flag (fp);
       //Iblis - 6/8/04 - No need to store/read the additional 4 values for item types
       //      //that do not need them.  This way I also don't have to update the version of
       //            //areas
       //
	...
	else pObjIndex->value[6] = fread_flag (fp);
...
//	      paf = reinterpret_cast<AFFECT_DATA *>(alloc_perm (sizeof (*paf)));
	  ...
	  else if (letter == 'S') //for super affect
	  {
  	    AFFECT_DATA *paf;
	    paf = alloc_perm (sizeof (*paf));
//	    paf = reinterpret_cast<AFFECT_DATA *>(alloc_perm (sizeof (*paf)));
		...
		paf->location = 1; //to avoid further bugs
	      }
	      else
	        paf->location = sn;
	    }
	    else paf->location =  fread_number (fp);
